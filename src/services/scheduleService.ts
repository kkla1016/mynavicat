...
export class ScheduleService {
  ...
  constructor(storageService: StorageService, backupService: BackupService) {
  ...
  public registerCronJob(schedule: any) {
    ...
    if (!cron.validate(schedule.CronExpression || schedule.cronExpression)) {
      return;
    }

    const task = cron.schedule(schedule.CronExpression || schedule.cronExpression, async () => {
  ...
  public async runScheduleBackup(schedule: any) {
    try {
  ...
  private cleanUpRetainedBackups(connectionId: number, retainCount: number) {
  ...
  private sendWindowsNotification(title: string, body: string) {
  ...
  public toggleSchedule(id: number, isEnabled: boolean): boolean {
    const schedules = this.storageService.getSchedules();
