...
# 1. 在新視窗啟動後端 Web API
Write-Host "[1/3] 正在啟動後端 Web API (Port 5000)..." -ForegroundColor Green
$backendCmd = "Set-Location '$rootDir\backend\MyNavicat.Api'; Write-Host '【MyNavicat 後端 API 服務】啟動中...' -ForegroundColor Green; dotnet run; Read-Host '按 Enter 鍵關閉視窗...'"
Start-Process powershell -ArgumentList "-NoExit", "-Command", $backendCmd

# 2. 在新視窗啟動前端 Vue 3 Dev Server
Write-Host "[2/3] 正在啟動前端 Vite Dev Server (Port 5173)..." -ForegroundColor Green
$frontendCmd = "Set-Location '$rootDir\frontend'; Write-Host '【MyNavicat 前端 UI 服務】啟動中...' -ForegroundColor Green; npm run dev; Read-Host '按 Enter 鍵關閉視窗...'"
...
for ($i = 1; $i -le $maxRetries; $i++) {
    ...
    if (-not $backendReady) {
        ...
        try {
            $async = $backendCheck.BeginConnect("127.0.0.1", 5000, $null, $null)
            if ($async.AsyncWaitHandle.WaitOne(300)) {
                $backendCheck.EndConnect($async)
                $backendReady = $true
                Write-Host "  -> 後端服務已就緒 (Port 5000)" -ForegroundColor DarkGreen
            }
